...
if (isset($_GET['n'])) {
    ...
    $n = isset($_SESSION['n_vector']) ? $_SESSION['n_vector'] : 0;
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Introducir Valores - Ejercicio 20</title>
...
<div class="contenedor-principal">
    ...
    <div class="cuerpo">
        ...
        <div class="contenido">
            <h2>Llenar Vectores</h2>
            <div class="formulario-box">
                <?php if (isset($error)) { echo "<p style='color:red;'>$error</p>"; } ?>
                
                <?php if ($n > 0) { ?>
                    <form action="multiplicar_vectores.php" method="POST">
                        <p><b>Vector 1 (1 x <?php echo $n; ?>):</b></p>
                        <div style="display: flex; gap: 5px; margin-bottom: 20px;">
                            <?php for ($i = 0; $i < $n; $i++) { ?>
                                <input type="number" name="v1[]" required style="width: 50px;">
                            <?php } ?>
                        </div>

                        <p><b>Vector 2 (<?php echo $n; ?> x 1):</b></p>
                        <div style="display: flex; flex-direction: column; gap: 5px;">
                            <?php for ($i = 0; $i < $n; $i++) { ?>
                                <input type="number" name="v2[]" required style="width: 50px;">
                            <?php } ?>
                        </div>
                ...
                <?php } else { ?>
                    <p style="color:red;">Error con el tamaño del vector.</p>
                <?php } ?>
                
                <br>
                <a href="../Ejercicio_20/form_vectores.html">Cambiar tamaño N</a>
            </div>
        </div>
    </div>
</div>
